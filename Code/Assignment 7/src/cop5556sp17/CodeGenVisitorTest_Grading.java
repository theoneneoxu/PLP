...
public class CodeGenVisitorTest_Grading {
	...
	public void imgMove2() throws Exception {
		...
		Runnable instance = CodeGenUtils.getInstance(name, bytecode, args);
		instance.run();
		String expectedLog = "getURL(https://s3.amazonaws.com/glocal-files/image/bi+(100).jpg)readFromURL(https://s3.amazonaws.com/glocal-files/image/bi+(100).jpg)createOrSetFrameshowImagegetScreenWidth" + ((int) Toolkit.getDefaultToolkit().getScreenSize().getWidth()) + "getScreenHeight" + ((int) Toolkit.getDefaultToolkit().getScreenSize().getHeight()) + "moveFrameshowImage";
		String actualLog = PLPRuntimeLog.getString();
		assertEquals(expectedLog, actualLog);
	}
