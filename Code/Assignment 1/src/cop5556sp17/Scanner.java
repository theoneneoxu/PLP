 ...
 */
	...
	public class Token {
		...
		LinePos getLinePos(){
			...
			LinePos linePos = new LinePos(lineNumber, columnNumber);
			return linePos;
		}
		...
		while (scanPosition <= codeLength) {
			...
			case START: {
				...
				default:
					if (isDigit(ch)) {
						...
						throw new IllegalCharException("Illegal char " + ch +" at position " + tokenStartPosition);
					}
					scanPosition++;
					break;
				}//switch (ch)
			...
			case AFTER_EQUAL:
				if (ch == '=') {
					...
					throw new IllegalCharException("Illegal char = at position " + tokenStartPosition);
				}
			...
			case IN_DIGIT: {
				if (isDigit(ch)) {
					...
					try {
						...
						throw new IllegalNumberException("Illegal number " + chars.substring(tokenStartPosition, scanPosition) +" at position " + tokenStartPosition);
					}
